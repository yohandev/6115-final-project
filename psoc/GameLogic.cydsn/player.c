...
void player_step() {
    ...
    // Pitch input is relative to aircraft's x axis
    quat pitch_rot = quat_angle_axis(pitch * Time.dt, quat_left(&Player.rot));
    ...
    // Integrate
    Player.yaw_pitch = quat_mul(&pitch_rot, &yaw_rot);
